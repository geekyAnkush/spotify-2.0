...
function Track({track, chooseTrack}) {
    ...
    return (
        ...
        hover:bg-white/10 px-4 py-2 rounded-lg group transition ease-out">
            <div className="flex item-center">
                <img src={track.albumUrl} alt="" className="rounded-xl h-12
                w-12 object-contain mr-3"/>
                <div>
                    <h4 className="text-white text-sm font-semibold truncate
                    w-[450px] ">{track.title}</h4>
            ...
            <div className="md:ml-auto flex items-center space-x-2.5">
                ...
                <h4 className="text-xs">{track.popularity? `${track.popularity}%` : ""}</h4>
                </div>
