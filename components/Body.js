...
function Body({ spotifyApi, chooseTrack }) {
    ...
    // const { accessToken } = session; //same as const accessToken = session.accessToken
    const accessToken = session?.accessToken;// optional chaining givis accesstoken when it exists
    const [search, setSearch] = useState("");
    const [searchResults,setSearchResults] = useState([]);
    const [newRealeases,setNewRealeases] = useState([]);
    ...
    useEffect(() => {
        ...
        spotifyApi.getNewReleases().then((res) =>{
            setNewRealeases(res.body.albums.items.map((track)=>{
                return {
                    id : track.id,
                    artist: track.artists[0].name,
                    title: track.name,
    ...
    return (
        <section className="bg-black ml-24 py-4 space-y-8 md:max-w-6xl flex-grow md:mr-2.5">
            ...
            <div className="grid overflow-y-scroll scrollbar-hide h-96 py-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-8 p-4">
              {searchResults.length === 0 ? newRealeases.slice(0,4).map((track)=>(
